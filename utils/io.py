...
def reencode_video_with_diff_fps(video_path: str, tmp_path: str, extraction_fps: float) -> str:
    ...
    try:
        ...
        stream = ffmpeg.filter(stream, 'fps', fps=extraction_fps)
        stream = ffmpeg.output(stream, new_path, loglevel='quiet')
        ffmpeg.run(stream)
    except Exception as e:
        with open('videochatgpt_path_fail.txt', 'a') as fp:
            fp.write(video_path + '\n')
        print(e)
    ...
    # (ffmpeg
    #     .input(video_path)
    #     .filter('fps', fpg=extraction_fps, round='up')
    #     .output(new_path)
    #     .global_args("-hide_banner") 
    #     .run(hide_banner=None)
    # )
